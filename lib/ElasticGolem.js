...
module.exports.prototype.save = function (record) {
	...
	return this.exists(record).then((exists) => {
		if (exists) {
			return new Promise.rejected( new DuplicateEntryError('during save of ' + this.createRecordId(record)) );
		} else {
			return this.client.index({
				index: this.options.elasticsearch.index,
				type: 'posting',
				body: {
					id: this.createRecordId(record),
					text: record.text,
					source: record.source,
					date: record.date
				}
			}).then(() => {
				this.logger.debug('Created record');
			}).catch((err) => {
				throw(err);
			});
		}
...
module.exports.prototype.exists = function (record) {
	this.logger.debug('Enter');
	...
	return this.client.search({
		...
		body: {
			...
			aggs: {
				duplicateCount: {
					terms: {
						field: this.createRecordId(record),
						min_doc_count: 2
					},
					aggs: {
						duplicateDocuments: {
		...
		var rv = 0 !== body.aggregations.duplicateCount.buckets.length;
		this.logger.debug('Leave exists with ', rv);
		return rv;
	});
};

module.exports.prototype.createRecordId = function (record) {
	if (! record.source || ! record.source) {
		throw new MissingPropertyError('createRecordId requires properties source and id');
	}
